...
- **Admin approval flow** — new organizations start as `PENDING`; a SUPERADMIN must approve before members can access the app
...
- **Team invites** — ORG_MANAGERs invite members by email with a scoped role; invites expire after 7 days
...
> Note: do **not** wrap values in quotes. The code strips surrounding quotes defensively, but it's cleaner to omit them.
...
The first user to register becomes `ORG_MANAGER` of a `PENDING` organization. To approve the org and promote yourself to SUPERADMIN, run this SQL in your Neon console:
...
Tests live in `apps/api/src/modules/**/__tests__/` and use Vitest with a mocked PrismaClient.
...
| ------ | ---------------------------- | ----------------------- | -------------------------------------------- |
| POST   | `/auth/register`             | public (Firebase token) | Register a new organization (starts PENDING) |
...
| GET    | `/assets`                    | VIEWER+                 | List assets (paginated, filterable)          |
