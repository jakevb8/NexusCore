...
- **NexusCoreDotNet EF/Npgsql type mapping rules** — whenever the Prisma schema changes, the corresponding EF mappings in NexusCoreDotNet's `AppDbContext.cs` must be updated following these rules:
  ...
  - **Native PG enum columns**: Prisma `enum` types (e.g., `Role`, `AssetStatus`, `OrgStatus`) → PostgreSQL native enum types. In `AppDbContext`, use `.HasColumnType("\"EnumName\"")` only (quoted, case-sensitive). Do NOT add `.HasConversion<string>()` — it overrides `MapEnum` and causes `column "status" is of type "AssetStatus" but expression is of type text` on every write. In `Program.cs`, `MapEnum<T>()` must be called in **two places**: (1) on `dataSourceBuilder` before `.Build()` — handles low-level ADO reads/writes; and (2) inside `UseNpgsql(dataSource, o => { o.MapEnum<T>(...) })` — wires EF's query translation layer. Calling it in only one place is not sufficient with EF 9 + external `NpgsqlDataSource`.
